...
*/
  ...
  async function Draw(){
    ...
    if(!t){
      ...
      burst = new Image()
      burst.src = "assets/images/temp/burst.png"
      ...
      starImgs = Array(9).fill().map((v,i) => {
        ...
        a.img.src = `https://epbalaban01.github.io/stars/star${i+1}.png`
        return a
      })
      ...
      tboard.src = 'assets/images/tXnnqzu.png'
      tboard_inactive = new Image()
      //tboard_inactive.src = 'tboard_inactive.jpg'
      tboard_inactive.src = 'assets/images/FhBEuaY.jpeg'
      gridoverlay = new Image()
      //gridoverlay.src = 'gridoverlay.png'
      gridoverlay.src = 'assets/images/QzF5K9b.png'
      border = new Image()
      //border.src = 'border.png'
      border.src = 'assets/images/yzLTMwN.png'
      ...
      drawDeath = (player, idx) => {
        ...
        if(Z>0){
          ...
          x.strokeStyle = '#0007'
          l = Q()
          x.strokeText('BİTTİ', l[0],l[1])
          x.fillText('BİTTİ', l[0]-sp*2,l[1]-sp*2)

          if(!idx){
            x.font=(fs=1e3*sp/Z) + 'px Courier Prime'
            x.fillStyle = '#fffc'
            x.strokeStyle = '#0007'
            x.strokeText('boşluğa bas', l[0],l[1]+fs*1.5)
            x.fillText('boşluğa bas', l[0]-sp*2,l[1]-sp*2+fs*1.5)
          }
        }
      }
      ...
      drawLeaderBoard = () => {
        ...
        if(Z>0){
          ...
          x.textAlign = 'center'
          l = Q()
          x.fillText('SIRALAMA',l[0]+18, l[1])
          x.fillStyle = '#fff'
    ...
    if(Z>0){
      ...
      x.font = (fs=1100/Z) + 'px Courier Prime'
      x.fillStyle = '#0f4'
      x.fillText('BAĞLANTILI OYUNCULAR',l[0],l[1])
    ...
    X = c.width - 129
    Y = c.height - 140
    renderButton('instadrop AI[✔]', X, Y, 'instadropu iptal et  ', 'toggleInstadrop()', 'rectangle', '#f008', '#4018', 24)

    X = c.width - 129
    Y = c.height - 140
    renderButton('instadrop AI[ ]', X, Y, 'instadropu etkinleştir(CPU YOĞUN OLUR!)  ', 'toggleInstadrop()', 'rectangle', '#0ff4', '#2088', 24)

    X = c.width - 100
    Y = c.height - 100
    renderButton('[tab] pause', X, Y, '[tab] oyunu duraklat/duraklatmayı kaldır  ', 'togglePause()', 'rectangle', '#0ff4', '#2088', 24)

    X = c.width - 115
    Y = c.height - 100
    renderButton('[tab] unpause', X, Y, '[tab] oyunu duraklat/duraklatmayı kaldır  ', 'togglePause()', 'rectangle', '#f008', '#4018', 24)
